...
O valor de $p = `r anova_ex[[4]][1]`$ mostrado na @tbl-anova acima é refere-se à área na distribuição $F$ que fica acima de $F_{calculado}$. Poderíamos tentar representar este valor visualmente na distribuição $F$, mas ele é tão pequeno, que a área em vermelho sequer aparece na figura.
...
i. estamos comparando pares em que os dois grupos têm $n = 8$ (BACK e LOW):

```{r}
#| echo: false
n = 8
q =  qtukey(p = 0.95, nmeans = 4, df = anova_ex[[1]][2])
DHS1 = q * sqrt((1/n + 1/n)*anova_ex[[3]][2])
```


$DHS =  `r round(q,3)`\sqrt{\left(\frac{1}{`r n`} + \frac{1}{`r n`}\right)`r anova_ex[[3]][2]`} = `r round(DHS1, 3)`$

ii. estamos comparando pares em que os dois grupos têm $n = 9$ (MED e HIGH):

```{r}
#| echo: false
n = 9
q =  qtukey(p = 0.95, nmeans = 4, df = anova_ex[[1]][2])
DHS2 = q * sqrt((1/n + 1/n)*anova_ex[[3]][2])
```


$DHS =  `r round(q,3)`\sqrt{\left(\frac{1}{`r n`} + \frac{1}{`r n`}\right)`r anova_ex[[3]][2]`} = `r round(DHS2, 3)`$


iii. estamos comparando pares em que um grupo tem $n = 8$ (BACK e LOW) e o outro $n = 9$ (MED e HIGH):
...
n2 = 9
q =  qtukey(p = 0.95, nmeans = 4, df = anova_ex[[1]][2])
DHS3 = q * sqrt((1/n1 + 1/n2)*anova_ex[[3]][2])
```


$DHS =  `r round(q,3)`\sqrt{\left(\frac{1}{`r n1`} + \frac{1}{`r n2`}\right)`r anova_ex[[3]][2]`} = `r round(DHS3, 3)`$

De todo modo, note que as diferenças ficam entre `r round(DHS2,3)` e `r round(DHS1,3)`.
...
+ $\overline{Y}_{`r Dm[1,1]`} = `r Dm[1,2]`$

+ $\overline{Y}_{`r Dm[2,1]`} = `r Dm[2,2]`$

+ $\overline{Y}_{`r Dm[3,1]`} = `r Dm[3,2]`$

+ $\overline{Y}_{`r Dm[4,1]`} = `r Dm[4,2]`$

E as diferenças ($\overline{Y}_{maior} - \overline{Y}_{menor}$) entre elas:

```{r}
#| echo: false
Ymean = tapply(medley$DIVERSITY, medley$ZINC, mean)
Ymean_D = as.matrix(dist(Ymean))
Ymean_D[upper.tri(Ymean_D)] = NA
data.frame(ZINC = colnames(Ymean_D), Ymean_D) %>% flextable()
```
